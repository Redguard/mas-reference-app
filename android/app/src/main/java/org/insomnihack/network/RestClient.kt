package org.insomnihack.network
...
class RestClient(private val secretKey: String) {
    ...
    private fun initSession (onSuccess: (String) -> Unit, onFailure: (IOException) -> Unit) {
        ...
        val b64PubKey = String (
            Base64.encode (
                org.insomnihack.RsaThingies().publicKey.encoded,
                Base64.NO_WRAP
            )
        )
    ...
    fun sendFeedback(name: String, feedback: String, onSuccess: (String) -> Unit, onFailure: (IOException) -> Unit) {
        ...
        initSession(
            {encryptedSessionId ->
                ...
                val sessionId = try {
                    val decrypted = String(org.insomnihack.RsaThingies().decryptData(encryptedSessionId))
